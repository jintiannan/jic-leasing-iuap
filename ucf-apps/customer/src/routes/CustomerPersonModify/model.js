...
export default {
    ...
    effects: {
        ...
        async loadList(param = {}, getState) {
            ...
            actions.customerPersonModify.updateState({showLoading: true});
            ...
            actions.customerPersonModify.updateState(updateData); // 更新数据和查询条件
        },
        ...
        async updateRowData(param={},getState){
            ...
            actions.customerPersonModify.updateState({list:_list});
        },
        async saveOrUpdate(formObj) {
            actions.customerPersonModify.updateState({showLoading: true});
            let data = processData(await api.save({data: formObj}));  // 调用 getList 请求数据
            actions.customerPersonModify.updateState({showLoading: false, isEdit: false});
        ...
        async delete (id) {
            ...
            actions.customerPersonModify.updateState({showLoading: false});
            if (data.success) {
        ...
        async getTreeNode() {
            ...
            actions.customerPersonModify.updateState({treeData: data});
        }
    }
};
