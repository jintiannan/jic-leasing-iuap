...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.projectPledge.updateState({powerButton:this.props.powerButton});
        actions.projectPledge.updateState({ifPowerBtn:this.props.ifPowerBtn});

        // actions.projectPledge.updateState({gridColumn:this.props.gridColumn});
        // actions.projectPledge.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.projectPledge.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.projectPledge.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.projectPledge.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.projectPledge.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.projectPledge.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.projectPledge.updateState({showModal : true, isEdit: true});
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {
            this.switchToCardView(param);
            actions.projectPledge.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        if(!this.props.error){
            ...
            actions.projectPledge.updateRowData({'record':_formObj});
            this.switchEdit();
        }
    }
