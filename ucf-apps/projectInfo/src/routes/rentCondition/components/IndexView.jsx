...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.rentCondition.updateState({powerButton:this.props.powerButton});
        actions.rentCondition.updateState({ifPowerBtn:this.props.ifPowerBtn});

        // actions.rentCondition.updateState({gridColumn:this.props.gridColumn});
        // actions.rentCondition.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.rentCondition.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.rentCondition.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.rentCondition.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.rentCondition.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.rentCondition.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.rentCondition.updateState({showModal : true,isEdit:true});
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {
            this.switchToCardView(param);
            actions.rentCondition.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        if(!this.props.error){
            ...
            actions.rentCondition.updateRowData({'record':_formObj});
            this.switchEdit();
        }
    }
