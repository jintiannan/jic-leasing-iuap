...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.receivePaymentOption.updateState({powerButton:this.props.powerButton});
        actions.receivePaymentOption.updateState({ifPowerBtn:this.props.ifPowerBtn});

        // actions.receivePaymentOption.updateState({gridColumn:this.props.gridColumn});
        // actions.receivePaymentOption.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.receivePaymentOption.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.receivePaymentOption.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.receivePaymentOption.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.receivePaymentOption.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.receivePaymentOption.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.receivePaymentOption.updateState({showModal : true});
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {
            this.switchToCardView(param);
            actions.receivePaymentOption.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        if(!this.props.error){
            ...
            actions.receivePaymentOption.updateRowData({'record':_formObj});
            this.switchEdit();
        }
    }
