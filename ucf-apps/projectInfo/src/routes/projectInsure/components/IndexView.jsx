...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.projectInsure.updateState({powerButton:this.props.powerButton});
        actions.projectInsure.updateState({ifPowerBtn:this.props.ifPowerBtn});

        // actions.projectInsure.updateState({gridColumn:this.props.gridColumn});
        // actions.projectInsure.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.projectInsure.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.projectInsure.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.projectInsure.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.projectInsure.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.projectInsure.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.projectInsure.updateState({showModal : true,isEdit:true});
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {
            this.switchToCardView(param);
            actions.projectInsure.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        if(!this.props.error){
            ...
            actions.projectInsure.updateRowData({'record':_formObj});
            this.switchEdit();
        }
    }
