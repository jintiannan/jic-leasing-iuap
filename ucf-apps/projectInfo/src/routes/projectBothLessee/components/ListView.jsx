...
class ListView extends Component {
    ...
    componentWillMount() {
        ...
        this.gridColumnOnTheLesseeAccount = [...genGridColumn(this.gridOnTheLesseeAccount)];
    }

    //组件生命周期方法-在第一次渲染后调用，只在客户端
    componentDidMount() {
        actions.projectBothLessee.loadList(this.props.queryParam);
    }
    ...
    freshData = (pageIndex) => {
        ...
        actions.projectBothLessee.loadList(queryParam);
    }
    ...
    //     }
    //     actions.projectBothLessee.updateRowData(param,index);
    //     actions.projectBothLessee.updateState({ selectedList : _selectedList });
    ...
    getSelectedDataFunc = (selectedList,record,index) => {
        ...
        if(_selectedList && _selectedList.length == 1){
            ...
            actions.projectBothLessee.updateState({ list2 : []});
        }


        actions.projectBothLessee.updateState({ list : _list,selectedList : _selectedList,formObject : _formObj});
    ...
    childList = (obj) => {
        console.log("进入" + obj);
        //加载子组件列表
        actions.projectBothLessee.loadChildList(this.props.queryParam);
    }
    ...
    render() {
        ...
        return (
            this.props.subForm === 'projectBoth'?
            <div className="grid-parent" style={{display:this.state.listView}}>
                ...
                <div>
                    ...
                    >
                        ...
                        <TabPane tab='客户账户信息' key="2">
                            <div>
                                <GridMain
                                    ref={(el) => this.gridOnTheLesseeAccount = el} //存模版
                                    columns={this.gridColumnOnTheLesseeAccount} //字段定义
