...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.penaltyRuleDetail.updateState({powerButton:this.props.powerButton});
        actions.penaltyRuleDetail.updateState({ifPowerBtn:this.props.ifPowerBtn});

        // actions.penaltyRuleDetail.updateState({gridColumn:this.props.gridColumn});
        // actions.penaltyRuleDetail.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.penaltyRuleDetail.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.penaltyRuleDetail.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.penaltyRuleDetail.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.penaltyRuleDetail.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.penaltyRuleDetail.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.penaltyRuleDetail.updateState({showModal : true,isEdit:true});
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {
            this.switchToCardView(param);
            actions.penaltyRuleDetail.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        if(!this.props.error){
            ...
            actions.penaltyRuleDetail.updateRowData({'record':_formObj});
            this.switchEdit();
        }
    }
